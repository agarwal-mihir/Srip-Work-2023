...
def plot_generic(x_train, y_train, x_cal, y_cal, add_to_plot=None, coef_1=0.3, coef_2=0.02):
...
# Function to plot multiple predictions from a trained model
def plot_multiple_predictions(x_train, y_train, x_cal, y_cal, x_test, y_preds, coef_1, coef_2):
    ...
    return plot_generic(x_train, y_train, x_cal, y_cal, add_multiple_predictions, coef_1, coef_2)

# Function to plot predictions from a trained model
def plot_predictions(x_train, y_train, x_cal, y_cal, x_test, y_preds, coef_1=0.3, coef_2=0.02):
    def add_predictions(ax):
        # Plot the neural network prediction curve as a line
        ax.plot(x_test, y_preds, '-', linewidth=3, label='neural net prediction')
    fig, ax = plot_generic(x_train, y_train, x_cal, y_cal, add_predictions, coef_1, coef_2)
    return fig, ax

# Function to plot uncertainty bands from a trained model
def plot_uncertainty_bands(x_train, y_train, x_cal, y_cal, x_test, y_preds, coef_1, coef_2):
    ...
    return plot_generic(x_train, y_train, x_cal, y_cal, add_uncertainty, coef_1, coef_2)
